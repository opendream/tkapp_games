...
var gameNameSound, correctSound, incorrectSound, themeSound, timeoutSound;
var isStop = false;
...
game.stop = function(){
  ...
  if(typeof incorrectSound != "undefined"){
    if(incorrectSound.isPlaying())
    incorrectSound.stop(); 
  }

  if(typeof themeSound != "undefined"){
    if(themeSound.isPlaying())
    themeSound.stop(); 
  }

  if(typeof timeoutSound != "undefined"){
    if(timeoutSound.isPlaying())
    timeoutSound.stop();
  }

  if(typeof sceneIntro != "undefined"){
    sceneIntro.removeAllChildren();
  }

  if(typeof sceneLevel != "undefined"){
    sceneLevel.removeAllChildren();
  }

  if(typeof sceneEasyPlay != "undefined"){
    sceneEasyPlay.removeAllChildren();
  }

  if(typeof sceneHardPlay != "undefined"){
    sceneHardPlay.removeAllChildren();
  }

  if(typeof sceneScore != "undefined"){
    sceneScore.removeAllChildren();
  }  
}
...
setupIntro = function(scene){
    ...
    gameNameSound = new lime.audio.Audio(soundPath + "game_start." + atype);
    setTimeout(function () {
      gameNameSound.play();
    }, 200);
...
spawnAnimationWithString = function (scene,s){
  correctSound = new lime.audio.Audio(soundPath + "game_correct." + atype);
  incorrectSound = new lime.audio.Audio(soundPath + "game_incorrect." + atype);
  var spawnImage;
  if (s == "correct"){
...
setupEasyGame = function(scene,callbackSummaryScore){
  themeSound = new lime.audio.Audio(soundPath + "game_play." + atype);
  timeoutSound = new lime.audio.Audio(soundPath + "game_timeout." + atype);
